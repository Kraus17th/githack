...
from githack.parser import parse
...
class Scanner:
    def __init__(self, base_url):
        ...
        self.domain = urllib.parse.urlparse(base_url).netloc.replace(":", "_")
    ...
    def is_valid_name(self, entry_name):
        if (
            ...
            or entry_name.startswith("\\")
            or not os.path.abspath(os.path.join(self.domain, entry_name)).startswith(
                self.dest_dir
            )
        ):
            try:
    ...
    def _request_data(url):
        ...
        return urllib.request.urlopen(request, context=context).read()
